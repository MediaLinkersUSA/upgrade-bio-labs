...
export async function POST(req: Request) {
  ...
  // Existence and the size selector are checked against the static catalog -
  // which sizes a product HAS never changes at request time - but stock does,
  // so that gate is applied below, after live pricing/stock is merged in.
  // Checking it here against the static file only would let a product just
  // marked out-of-stock in WooCommerce keep selling until the next deploy.
  ...
  //
  // withLivePricing also merges in live stock status (falling back to the
  // static inStock if Woo is unreachable), so the gate right after this is
  // checking the same freshly-fetched value the customer would have seen.
  const items = (
    await Promise.all(
  ...
  ).filter((i) => i.product.inStock);

  if (!items.length) {
    return NextResponse.json(
      { error: "Everything in your cart just went out of stock." },
      { status: 422 }
    );
  }
